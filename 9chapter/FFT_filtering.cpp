...
void passFiltering(Mat dft_coef, Mat filter, Mat dst[2])
{
    ...
    multiply(dft_coef, filter, filtered_mat);
    log_mag(filtered_mat, dst[0]);

    shuffling(filtered_mat, filtered_mat);
    FFT_2D(filtered_mat, filtered_img, -1);
...
int main()
{
    Mat image = imread("./9chapter/image/filter_test.jpg", 0);
