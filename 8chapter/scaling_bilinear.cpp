...
int main()
{
    Mat image = imread("./8chapter/image/interpolation_test.jpg", 0);        // 명암도 영상 로드
    ...
    imshow("image", image);
    imshow("dst1_bilinear", dst1);
    imshow("dst2_nearset", dst2);
    imshow("OpenCV_bilinear", dst3);
    imshow("OpenCV_nearset", dst4);
    waitKey();
    return 0;
}
